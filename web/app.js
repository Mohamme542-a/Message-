...
function renderActivation() {
  app.innerHTML = `<section class="auth-shell ready-exterior"><div class="exterior-orb orb-one"></div><div class="exterior-orb orb-two"></div><div class="auth-card exterior-card"><div class="cover-frame">${mark()}</div><p class="exterior-kicker">ALPHA BYTE · PRIVATE MESSENGER</p><h1 class="auth-title">Alpha Byte</h1><p class="auth-copy">أدخل رمز التفعيل الممنوح لك لفتح هذا الجهاز.</p><form class="form exterior-form" id="activation-form"><label class="field">رمز التفعيل<input id="activation-code" type="password" inputmode="text" autocomplete="one-time-code" autofocus required /></label><p class="notice">${text(state.notice)}</p><button class="primary" type="submit"><span>متابعة آمنة</span><span class="arrow">‹</span></button></form><div class="exterior-trust"><span>⌁</span><p>رمز التفعيل يتحقق منه الخادم ولا يُحفظ داخل التطبيق.</p></div></div></section>`;
...
function renderAccess() {
  ...
  app.innerHTML = `<section class="auth-shell ready-exterior"><div class="exterior-orb orb-one"></div><div class="exterior-orb orb-two"></div><div class="auth-card exterior-card"><div class="access-cover"><div class="cover-frame compact">${mark()}</div><div><p class="exterior-kicker">ALPHA BYTE</p><h1>هويتك الخاصة</h1><p>مراسلة محلية التشفير قبل الإرسال.</p></div></div><div class="switch exterior-switch"><button class="${login ? "selected" : ""}" id="login-tab" type="button">دخول</button><button class="${!login ? "selected" : ""}" id="register-tab" type="button">إنشاء هوية</button></div><form class="form exterior-form" id="access-form"><label class="field">اسم المستخدم<input id="username" autocomplete="username" placeholder="username" required /></label><label class="field">العبارة السرية<input id="secret" type="password" autocomplete="${login ? "current-password" : "new-password"}" placeholder="12 حرفًا على الأقل" minlength="12" required /></label><p class="notice">${text(state.notice)}</p><button class="primary" type="submit"><span>${login ? "فتح حسابي" : "إنشاء حسابي"}</span><span class="arrow">‹</span></button></form><button class="link-button exterior-link" id="change-code" type="button">تغيير رمز التفعيل</button><div class="exterior-trust"><span>⌁</span><p>لا نطلب رقم هاتف أو بريدًا إلكترونيًا.</p></div></div></section>`;
