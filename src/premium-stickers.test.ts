...
describe("Alpha Byte premium stickers", () => {
  it("contains distinct, renderable sticker assets", () => {
    expect(PREMIUM_STICKERS).toHaveLength(35);
    expect(new Set(PREMIUM_STICKERS.map((sticker) => sticker.id)).size).toBe(35);
    expect(new Set(PREMIUM_STICKERS.map((sticker) => sticker.src)).size).toBe(35);
