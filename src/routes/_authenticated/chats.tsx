...
function ChatsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: profileData } = await profilesTable
        ...
        .in("id", peerIds);
      const profiles = profileData as Array<{ id: string; username: string; display_name: string; avatar_url: string | null; is_verified: boolean; last_seen: string | null; show_online: boolean; show_last_seen: boolean; identity_public_key: string | null }> | null;
  ...
  async function handleCreateGroup() {
    ...
    try {
      const created = await createGroup(vault.identityPrivateKey, vault.identityPublicKey, {
        title: groupTitle.trim(),
        description: groupDescription.trim(),
        kind: groupKind,
      });
      if (!created?.id) throw new Error("GROUP_CREATION_FAILED");
      ...
      if (message.includes("INVALID_GROUP_DETAILS")) toast.error("تحقق من اسم المجموعة أو القناة والوصف.");
        ...
        <section className="mb-5">
          ...
          <ul className="space-y-2">
            {(groupRows ?? []).map(({ group, role }) => (
              <li key={group.id}><Link to="/group/$id" params={{ id: group.id }} className="flex min-h-[76px] items-center gap-3 rounded-2xl border border-border glass px-4 py-3.5 press">
                {group.avatar_url ? <img src={group.avatar_url} alt="" className="premium-avatar-frame h-14 w-14 shrink-0 rounded-2xl object-cover" /> : <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl brand-bg text-primary-foreground"><Users className="h-5 w-5" /></span>}
          ...
          {rows.map(({ conversation, peer, latest, preview }) => (
            <li key={conversation.id}>
              <Link
                ...
                className="flex min-h-[76px] items-center gap-3 rounded-2xl border border-border glass px-4 py-3.5 press"
              >
                {peer?.avatar_url ? <img src={peer.avatar_url} alt="" className="premium-avatar-frame h-14 w-14 shrink-0 rounded-2xl object-cover" /> : <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl brand-bg text-sm font-bold text-primary-foreground">{(peer?.display_name || peer?.username || "?").slice(0, 2).toUpperCase()}</span>}
                <span className="min-w-0 flex-1">
                  ...
                  <span className="flex items-center gap-1 truncate text-[11px] text-muted-foreground"><span className={peer && isOnline(peer) ? "h-1.5 w-1.5 rounded-full bg-emerald-500" : "h-1.5 w-1.5 rounded-full bg-muted-foreground/50"} />{peer ? formatLastSeen(peer) : "غير متصل"}</span>
