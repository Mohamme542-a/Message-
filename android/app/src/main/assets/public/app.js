...
function renderConversation() {
  ...
  const reactions = ["👍", "❤️", "😂", "😮", "😢", ...(isGranted("sticker_pack") ? ["✨", "🔥", "💎", "🚀", "👑"] : [])];
  return `<section class="conversation-page"><div class="conversation-head"><button class="back-button" id="back-inbox" type="button">›</button><div><strong>${text(name)}</strong><small>تشفير محلي قبل الإرسال</small></div><button class="icon-button small" id="reaction-button" type="button">☺</button></div>${state.reactionPicker ? `<div class="reaction-picker">${reactions.map(emoji => `<button type="button" data-reaction="${emoji}">${emoji}</button>`).join("")}</div>` : ""}<div class="message-scroll">${items}</div><form class="composer" id="message-form"><input id="message-text" placeholder="رسالة مشفّرة" autocomplete="off" /><select id="expiry"><option value="day" ${state.expiry === "day" ? "selected" : ""}>يوم</option><option value="week" ${state.expiry === "week" ? "selected" : ""}>أسبوع</option><option value="month" ${state.expiry === "month" ? "selected" : ""}>شهر*</option></select><label class="attach" title="اختيار ملف">＋<input id="attachment-file" type="file" hidden /></label><button class="attach camera" id="camera-button" type="button" title="التقاط صورة">◉</button><input id="camera-file" type="file" accept="image/*" capture="environment" hidden /><button class="send" type="submit">↑</button></form><p class="retention-note">* تحتفظ الأجهزة بنسخها وفق اختيارها، لكن الخادم يحذف كل النسخ خلال أسبوع.</p></section>`;
}
